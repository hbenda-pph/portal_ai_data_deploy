// includes/call_intelligence/vw_lost_opportunity.js
module.exports = (companyId, projectId, rawDataset) =>
  publish("vw_lost_opportunity", {
    type: "view",
    database: projectId,
    schema: "gold",
  ...
  SELECT
    ...
    f.company_id,
    f.call_received_on,
    \`pph-central.settings.fn_convert_utc_localtz\`(f.call_received_on, ${companyId}) AS call_received_on_local,
    DATETIME_DIFF(CURRENT_DATETIME(), DATETIME(\`pph-central.settings.fn_convert_utc_localtz\`(f.call_received_on, ${companyId})), HOUR) AS hours_since_call,
