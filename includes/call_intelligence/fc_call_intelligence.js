// includes/call_intelligence/fc_call_intelligence.js
module.exports = (companyId, projectId, rawDataset) =>
  publish("fc_call_intelligence", {
    type: "table",
    database: projectId,
    schema: "gold",
    description: "Canonical call intelligence fact table reconciling ServiceTitan calls, jobs, customer data, and Gemini AI transcriptions",
    tags: ["gold", "call_intelligence", "fc_call_intelligence"],
    dependencies: [`${projectId}.gold.dm_service_benchmarks`],
    bigquery: {
      partitionBy: "DATE(call_received_on)",
      clusterBy: ["call_outcome", "lost_reason_category", "business_unit_id", "agent_id"]
    }
  })
    .query(`
  WITH dedup_calls AS (
    ...
    FROM \`${projectId}.silver.vw_call\` c
  ...
  dedup_recordings AS (
    ...
    FROM \`${projectId}.silver.tb_call_recordings\`
  ...
  open_estimates AS (
    ...
    FROM \`${projectId}.silver.vw_estimate\`
    WHERE status_name IN ('Open', 'Dismissed') AND subtotal > 0
  ),
  enriched AS (
    SELECT
      TO_HEX(SHA256(CONCAT(CAST(COALESCE(c.company_id, ${companyId}) AS STRING), '_', CAST(c.lead_call_id AS STRING)))) AS call_sk,
      c.lead_call_id,
      COALESCE(c.company_id, ${companyId}) AS company_id,
      c.call_received_on,
    ...
    LEFT JOIN \`${projectId}.silver.vw_customer\` cust ON c.customer_id = cust.id
    LEFT JOIN \`${projectId}.bronze.campaign\` cmp ON c.campaign_id = cmp.id
    LEFT JOIN \`${projectId}.bronze.business_unit\` bu ON c.business_unit_id = bu.id
    ...
    LEFT JOIN \`${projectId}.gold.dm_service_benchmarks\` bm ON t.service_requested_category = bm.service_category
  )
