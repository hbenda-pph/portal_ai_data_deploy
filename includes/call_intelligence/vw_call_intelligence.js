// includes/call_intelligence/vw_call_intelligence.js
module.exports = (companyId, projectId, rawDataset) =>
  publish("vw_call_intelligence", {
    type: "view",
    database: projectId,
    schema: "gold",
    description: "Real-time canonical call intelligence view reconciling ServiceTitan calls, jobs, customer data, and Gemini AI transcriptions in live mode",
    tags: ["gold", "call_intelligence", "vw_call_intelligence"],
    dependencies: [`${projectId}.gold.vw_service_benchmarks`]
  })
    .query(`
  WITH dedup_calls AS (
    SELECT 
  ...
  enriched AS (
    ...
    LEFT JOIN \`${projectId}.gold.vw_service_benchmarks\` bm ON t.service_requested_category = bm.service_category
  )
